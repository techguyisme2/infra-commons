...
## Dev loop
```bash
python -m pytest -q tests            # 23+ mechanism tests, stdlib+pytest only
...
verify the pinned ref's `tofu_runner/` tree matches the checkout at import,
so any code change needs a new tag; doc-only commits don't.
